...
function ICs!(phi_M1n,pi_M1n,gttn,gxxn,gxyn,gxzn,gyyn,gyzn,gzzn,sqrtmingn)
    @inbounds Threads.@threads for iy=1:Ny0
        @inbounds for iz=1:Nz0
            ...
            #Metric components for f(r)=1-2mr^2/(r^3+2l^2m)
            ...
            #Minus square root of the determinant of the metric for f(r)=1-2mr^2/(r^3+2l^2m)
            ...
            # #Metric components for f(r)=1-r^2/(a+br^4)
            ...
            # #Minus square root of the determinant of the metric for f(r)=1-r^2/(a+br^4)
            # sqrtmingn[iy,iz]=(8.0*(cos((pi*0.0)/2.0)^2.0*cos((pi*ys[iy])/2.0)^2.0*cos((pi*zs[iz])/2.0)^2.0)/pi^3.0)^(-1.0);

            #println("Test")
            #println(sqrtmingn[iy,iz])
